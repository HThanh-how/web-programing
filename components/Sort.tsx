...
export default function Home() {
  ...
  return (
    <Box p={15}>
      ...
      <Text p={4} >Sorted Numbers Max to Min: {sortedNumbers.join(', ')}</Text>
      <Text p={4}>Sorted Numbers Min to Max: {sortedNumbers.reverse().join(', ')}</Text>
    <Text p={4} >Min: {sortedNumbers[0]} </Text>
    <Text p={4} >Max: {sortedNumbers.reverse()[0]} </Text>
    </Box>
  );
}
